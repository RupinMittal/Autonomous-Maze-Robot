...
//global variables (so they show up in Live Expressions debugging interface)
	...
	const float kP_distance = 0.25;					//how aggresively we want to respond to distance errors
	const float kD_distance = 0.4;					//factor for responding to how close we are to reaching our target distance

	const float kP_angle = 0.25;						//how aggresively we want to respond to angle errors
	const float kD_angle = 0.4;					//factor for responding to how close we are to reaching our target angle
	...
	const int NUM_ZERO_ERROR_FOR_GOAL = 75; 	//number of calls with close to zero error required for goal to be met
	const double zero_error = 5; 				//max error that is considered sufficiently close to zero error
