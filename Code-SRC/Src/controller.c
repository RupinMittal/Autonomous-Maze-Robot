...
void move(int8_t n)
{
	...
	resetPID();
	setPIDGoalA(0);
	setPIDGoalD(n * -582);		//the difference in encoder counts for a 90 degree turn is approximately 600 encoder counts. (as I measured it using live expressions for encoder counts)
...
void turn(int8_t n)
{
	...
	resetPID();
	setPIDGoalA(n * 605);		//the difference in encoder counts for a 90 degree turn is approximately 620 encoder counts. (as I measured it using live expressions for encoder counts)
	setPIDGoalD(0);
	while (PIDdone() != 1)		//wait until turn is complete
		;
	resetPID();					//and then reset
